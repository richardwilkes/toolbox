...
func TestLineIntersection(t *testing.T) {
	...
	a1 := geom.NewPoint(0.0, 0.0)
	a2 := geom.NewPoint(10.0, 10.0)
	b1 := geom.NewPoint(0.0, 10.0)
	b2 := geom.NewPoint(10.0, 0.0)

	intersection := geom.LineIntersection(a1, a2, b1, b2)
	c.Equal(1, len(intersection))
	c.Equal(5.0, intersection[0].X)
	c.Equal(5.0, intersection[0].Y)

	// Test parallel lines (no intersection)
	c1 := geom.NewPoint(0.0, 0.0)
	c2 := geom.NewPoint(10.0, 0.0)
	d1 := geom.NewPoint(0.0, 5.0)
	d2 := geom.NewPoint(10.0, 5.0)

	noIntersection := geom.LineIntersection(c1, c2, d1, d2)
	c.Equal(0, len(noIntersection))

	// Test identical points
	e1 := geom.NewPoint(5.0, 5.0)
	e2 := geom.NewPoint(5.0, 5.0)
	f1 := geom.NewPoint(5.0, 5.0)
	f2 := geom.NewPoint(5.0, 5.0)

	identicalIntersection := geom.LineIntersection(e1, e2, f1, f2)
	c.Equal(1, len(identicalIntersection))
	c.Equal(5.0, identicalIntersection[0].X)
	c.Equal(5.0, identicalIntersection[0].Y)

	// Test point intersecting with line
	g1 := geom.NewPoint(5.0, 5.0)
	g2 := geom.NewPoint(5.0, 5.0) // Same point
	h1 := geom.NewPoint(0.0, 5.0)
	h2 := geom.NewPoint(10.0, 5.0)

	pointLineIntersection := geom.LineIntersection(g1, g2, h1, h2)
	c.Equal(1, len(pointLineIntersection))
	c.Equal(5.0, pointLineIntersection[0].X)
	c.Equal(5.0, pointLineIntersection[0].Y)

	// Test lines that don't intersect within their segments
	i1 := geom.NewPoint(0.0, 0.0)
	i2 := geom.NewPoint(1.0, 1.0)
	j1 := geom.NewPoint(2.0, 0.0)
	j2 := geom.NewPoint(3.0, 1.0)
...
func TestLineIntersectionOverlapping(t *testing.T) {
	...
	a1 := geom.NewPoint(0.0, 0.0)
	a2 := geom.NewPoint(10.0, 0.0)
	b1 := geom.NewPoint(5.0, 0.0)
	b2 := geom.NewPoint(15.0, 0.0)
	...
	for _, pt := range overlapping {
		if pt.X == 5.0 && pt.Y == 0.0 {
			foundStart = true
		}
		if pt.X == 10.0 && pt.Y == 0.0 {
			foundEnd = true
		}
	}
	c.True(foundStart)
...
func TestPointSegmentDistanceSquared(t *testing.T) {
	...
	c.Equal(0.0, distanceSquared1)

	// Test point perpendicular to the line segment
	p2 := geom.NewPoint(5.0, 3.0)
	distanceSquared2 := geom.PointSegmentDistanceSquared(s1, s2, p2)
	c.Equal(9.0, distanceSquared2) // 3^2 = 9

	// Test point closest to an endpoint
	p3 := geom.NewPoint(-2.0, 4.0)
	distanceSquared3 := geom.PointSegmentDistanceSquared(s1, s2, p3)
	c.Equal(20.0, distanceSquared3) // (-2)^2 + 4^2 = 4 + 16 = 20

	// Verify consistency with PointSegmentDistance
	s3 := geom.NewPoint(1.0, 2.0)
	s4 := geom.NewPoint(5.0, 6.0)
	p4 := geom.NewPoint(3.0, 1.0)
