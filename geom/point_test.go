...
func TestConvertPoint(t *testing.T) {
	...
	intPoint := geom.NewPoint(3, 4)
	floatPoint := geom.ConvertPoint[float64](intPoint)
	c.Equal(3.0, floatPoint.X)
	c.Equal(4.0, floatPoint.Y)

	// Convert from float to int
	floatPoint2 := geom.NewPoint(3.7, 4.9)
	intPoint2 := geom.ConvertPoint[int](floatPoint2)
	c.Equal(3, intPoint2.X)
	c.Equal(4, intPoint2.Y)
}
...
func TestPointDiv(t *testing.T) {
	...
	c.Equal(3.5, resultf.X)
	c.Equal(4.5, resultf.Y)
}
...
func TestPointFloor(t *testing.T) {
	...
	c.Equal(3.0, result.X)
	c.Equal(4.0, result.Y)

	// Test with negative values
	p2 := geom.NewPoint(-3.7, -4.2)
	result2 := p2.Floor()

	c.Equal(-4.0, result2.X)
	c.Equal(-5.0, result2.Y)
}
...
func TestPointCeil(t *testing.T) {
	...
	c.Equal(4.0, result.X)
	c.Equal(5.0, result.Y)

	// Test with negative values
	p2 := geom.NewPoint(-3.7, -4.2)
	result2 := p2.Ceil()

	c.Equal(-3.0, result2.X)
	c.Equal(-4.0, result2.Y)
}
