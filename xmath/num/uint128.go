...
// MarshalJSON implements json.Marshaler. Note that this intentionally
// generates a string to ensure the correct value is retained.
func (u Uint128) MarshalJSON() ([]byte, error) {
	return []byte(`"` + u.String() + `"`), nil
}

// UnmarshalJSON implements json.Unmarshaler.
func (u *Uint128) UnmarshalJSON(data []byte) (err error) {
	if data[0] == '"' {
		if data[len(data)-1] != '"' {
	...
	v, err := Uint128FromString(string(data))
	if err != nil {
		return err
	}
	*u = v
...
// MarshalYAML implements yaml.Marshaler. Note that this intentionally
// generates a string to ensure the correct value is retained.
