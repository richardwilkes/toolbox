...
// Union returns the union of both polygons. Does not handle self-intersecting polygons. Use Simplify() first, if
// necessary.
func (p Polygon[T]) Union(other Polygon[T]) Polygon[T] {
	...
	for !q.empty() {
		...
		if e.left {
			...
			if e.edgeType == edgeSameTransition || (e.edgeType == edgeNormal && !e.other.inside) {
				c.add(e.segment())
			}
			if otherPos != -1 {
				s.remove(s[otherPos])
