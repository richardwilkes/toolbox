...
func Rect(bounds geom32.Rect) Polygon {
	return Polygon{Contour{
		bounds.Point,
		geom32.Point{X: bounds.X, Y: bounds.Bottom() - 1},
		geom32.Point{X: bounds.Right() - 1, Y: bounds.Bottom() - 1},
		geom32.Point{X: bounds.Right() - 1, Y: bounds.Y},
	}}
}
...
func (p Polygon) Bounds() geom32.Rect {
	if len(p) == 0 {
		return geom32.Rect{}
	}
	b := p[0].Bounds()
...
func (p Polygon) Contains(pt geom32.Point) bool {
	for i := range p {
		if p[i].Contains(pt) {
			return true
		}
...
func (p Polygon) identifyNonContributingContours(op clipOp, clip Polygon) (subjNonContributing, clipNonContributing []bool) {
	...
	if (op == intersectOp || op == subtractOp) && len(p) > 0 && len(clip) > 0 {
		...
		boxes := make([]geom32.Rect, len(clip))
		for i, c := range clip {
			boxes[i] = c.Bounds()
		}
		for si := range p {
