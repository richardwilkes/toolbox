...
func F64d2FromStringForced(str string) F64d2 {
	f, _ := F64d2FromString(str) //nolint:errcheck // failure results in 0, which is acceptable here
	return f
}
...
func (f F64d2) CommaWithSign() string {
	if f >= 0 {
		return "+" + f.Comma()
	}
	return f.Comma()
...
func (f F64d2) StringWithSign() string {
	if f >= 0 {
		return "+" + f.String()
	}
	return f.String()
...
func (f *F64d2) UnmarshalText(text []byte) error {
	f1, err := F64d2FromString(unquote(text))
	if err != nil {
		return err
	}
	*f = f1
...
func (f *F64d2) UnmarshalJSON(in []byte) error {
	v, err := F64d2FromString(unquote(in))
	if err != nil {
		return err
	}
	*f = v
