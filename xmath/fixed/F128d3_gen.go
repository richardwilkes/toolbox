...
var (
	F128d3One     = F128d3FromInt(1)
	F128d3Half    = F128d3FromStringForced("0.5")
	F128d3NegHalf = F128d3Half.Neg()
)

// F128d3 holds a fixed-point value that contains up to 3 decimal places. Values are truncated, not rounded.
type F128d3 struct {
	data num.Int128
}
...
// F128d3FromFloat32 creates a new F128d3 value from a float32.
func F128d3FromFloat32(value float32) F128d3 {
	return F128d3FromFloat64(float64(value))
}
...
// F128d3FromInt creates a new F128d3 value from an int.
func F128d3FromInt(value int) F128d3 {
	return F128d3FromInt64(int64(value))
}
...
func (f F128d3) Ceil() F128d3 {
	v := f.Trunc()
	if f.GreaterThan(F128d3{}) && f != v {
		v = v.Add(F128d3One)
	}
	return v
}
...
func (f F128d3) Round() F128d3 {
	value := f.Trunc()
	rem := f.Sub(value)
	if rem.GreaterThanOrEqual(F128d3Half) {
		value = value.Add(F128d3One)
	} else if rem.LessThan(F128d3NegHalf) {
		value = value.Sub(F128d3One)
	}
	return value
}

// Min returns the minimum of this value or its argument.
func (f F128d3) Min(value F128d3) F128d3 {
...
func (f F128d3) Max(value F128d3) F128d3 {
...
func (f F128d3) AsInt() int {
	return int(f.data.Div(multiplierF128d3).AsInt64())
}

// Int is the same as AsInt(), except that it returns an error if the value cannot be represented exactly with an int.
func (f F128d3) Int() (int, error) {
	n := f.AsInt()
	if F128d3FromInt(n) != f {
...
func (f F128d3) AsFloat32() float32 {
	f64, _ := new(big.Float).SetPrec(128).Quo(f.data.AsBigFloat(), multiplierF128d3BigFloat).Float32()
	return f64
}

// Float32 is the same as AsFloat32(), except that it returns an error if the value cannot be represented exactly with a
// float32.
func (f F128d3) Float32() (float32, error) {
	n := f.AsFloat32()
...
func (f F128d3) CommaWithSign() string {
	if f.data.Sign() >= 0 {
		return "+" + f.Comma()
	}
	return f.Comma()
...
func (f F128d3) StringWithSign() string {
	if f.data.Sign() >= 0 {
		return "+" + f.String()
	}
	return f.String()
