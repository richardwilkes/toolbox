...
// F128d16 holds a fixed-point value that contains up to 16 decimal places.
...
func F128d16FromString(str string) (F128d16, error) {
	...
	default:
		if _, ok := value.SetString(parts[0], 10); !ok {
			return F128d16{}, errs.New("invalid value")
		}
		if value.Sign() < 0 {
			neg = true
			value.Neg(value)
...
func (f *F128d16) UnmarshalJSON(data []byte) error {
	if data[0] == '"' {
		if data[len(data)-1] != '"' {
	...
	v, err := F128d16FromString(string(data))
	if err != nil {
		return err
	}
	*f = v
