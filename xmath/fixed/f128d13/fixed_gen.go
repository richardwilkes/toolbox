...
var (
	multiplierBigInt   = new(big.Int).Exp(big.NewInt(10), big.NewInt(13), nil)
...
// Int holds a fixed-point value that contains up to 13 decimal places. Values are truncated, not rounded.
type Int struct {
	data num.Int128
}
...
func FromFloat64(value float64) Int {
	f, _ := FromString(new(big.Float).SetPrec(128).SetFloat64(value).Text('f', 14)) //nolint:errcheck // Failure means 0
	return f
}
