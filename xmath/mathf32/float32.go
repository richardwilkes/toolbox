...
// Floating-point limit values. Max is the largest finite value representable by the type. SmallestNonzero is the
// smallest positive, non-zero value representable by the type.
const (
...
func Lgamma(x float32) (lgamma float32, sign int) {
	var f64 float64
	f64, sign = math.Lgamma(float64(x))
	return float32(f64), sign
}
...
func NaN() float32 {
	return math.Float32frombits(0x7FF80001)
}
